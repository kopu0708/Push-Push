...
- [✅] **[목표 4] 2인 로컬/네트워크 멀티플레이어 기능 구현**
  ...
  본 프로젝트는 기존 2D 물리 기반 밀어내기 싱글 게임에서 Netcode for GameObjects를 이용한 네트워크 멀티플레이어(2인용)로 확장 중입니다.
  씬 이동 흐름: Scene_Main (메인) ➡️ Scene_Lobby (네트워크 로비) ➡️ Scene_Multi (실제 인게임 전장)

  로비 시스템: LobbyManager.cs를 통해 방장(Host)과 참가자(Client)가 분기되어 접속합니다.

  유동적 IP 주소 입력: 빌드를 매번 새로 하지 않아도 되도록 UI 입력창(TMP_InputField)에 방장의 IP를 적어 실시간으로 UnityTransport 주소를 덮어쓰는 방식을 적용   했습니다.
